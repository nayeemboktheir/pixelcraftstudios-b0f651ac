...
serve(async (req) => {
  ...
  try {
    ...
    if (isManual) {
      emailSubject = body.product_name;
      emailHtml = `
        <!DOCTYPE html>
        <html>
        <head><meta charset="utf-8"><title>${body.product_name}</title></head>
        <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f5f5f5;">
          ...
          <div style="background: white; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
            <div style="white-space: pre-wrap; font-size: 15px; color: #444;">${body.custom_message}</div>
            ${body.download_link ? `
              <div style="text-align: center; margin: 30px 0;">
                <a href="${body.download_link}" style="display: inline-block; background: linear-gradient(135deg, #0f3460 0%, #1a1a2e 100%); color: white; text-decoration: none; padding: 16px 40px; border-radius: 12px; font-size: 16px; font-weight: 700;">📥 ডাউনলোড করুন</a>
              </div>
            ` : ''}
          </div>
          <div style="text-align: center; padding: 20px; color: #888; font-size: 12px; background: white; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 16px 16px;">
            <p style="margin: 0;">&copy; ${new Date().getFullYear()} ${shopName}</p>
          </div>
        </body>
        </html>
      `;
    } else {
      emailSubject = `🎉 আপনার প্রোডাক্ট রেডি - Order #${body.order_number}`;
      emailHtml = `
        <!DOCTYPE html>
        <html>
        <head><meta charset="utf-8"><title>Your Digital Product is Ready!</title></head>
        <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f5f5f5;">
          <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%); color: white; padding: 30px; border-radius: 16px 16px 0 0; text-align: center;">
            ${settingsMap.shop_logo_url ? `<img src="${settingsMap.shop_logo_url}" alt="${shopName}" style="max-height: 50px; margin-bottom: 15px;" />` : ''}
          ...
          <div style="background: white; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
            <p style="font-size: 16px; margin-top: 0;">প্রিয় <strong>${body.customer_name || 'Customer'}</strong>,</p>
            ...
            <div style="background: #f8f9fa; border-radius: 12px; padding: 20px; margin: 25px 0; border: 1px solid #e9ecef;">
              ...
              <p style="margin: 5px 0; color: #666;">মোট: ৳${body.total || 0}</p>
            </div>
            <div style="text-align: center; margin: 30px 0;">
              <a href="${body.download_link}" style="display: inline-block; background: linear-gradient(135deg, #0f3460 0%, #1a1a2e 100%); color: white; text-decoration: none; padding: 16px 40px; border-radius: 12px; font-size: 16px; font-weight: 700; letter-spacing: 0.5px;">📥 ডাউনলোড করুন</a>
            </div>
            <div style="background: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; padding: 15px; margin-top: 20px;">
              <p style="margin: 0; font-size: 13px; color: #856404;">⚠️ <strong>গুরুত্বপূর্ণ:</strong> এই ডাউনলোড লিংকটি শুধুমাত্র আপনার জন্য। অনুগ্রহ করে অন্যদের সাথে শেয়ার করবেন না।</p>
            </div>
          </div>
